#pragma once

void removeProduct(const DataNeeded* internalData, Sector* superEDA, Storage* supermarketStorage); //topic 3.1
void updatePrice(Storage* supermarketStorage); // topic 3.2
void discount(const DataNeeded* internalData, Sector* superEDA, Storage* supermarketStorage); // topic 3.3
void saveSuperMarket(const DataNeeded* internalData, const Sector* SuperEDA, const Storage* supermarketStorage, const Filepaths* supermarketFilepaths); // topic 3.4
void loadSuperMarket(DataNeeded* internalData, Sector* SuperEDA, Storage* supermarketStorage, Filepaths* supermarketFilepaths); // topic 3.5
void printProducts(DataNeeded* internalData, Sector* superEDA, Storage* supermarketStorage); // topic 3.6
void createNewArea(DataNeeded* internalData); // topic 3.7
void showSalesRegister(DataNeeded* internalData, Sector* superEDA); // topic 3.8
void changeArea(DataNeeded* inteirnData, Sector* superEDA); // topic 3.9
void managementMenu (DataNeeded* internalData, Sector* SuperEDA, Storage* supermarketStorage, Filepaths* supermarketFilepaths);
